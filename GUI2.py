...
def call_local_model(image):
    try:
        img_array = np.array(image)

        results = model.predict(img_array, device='cuda')[0]
        ...
        print(f"Model inference error occurred: {err}")
        return None
...
def generate_frames(skip_frames=2):
    ...
    while cap and cap.isOpened():
        ...
        result = call_local_model(pil_image)

        if result is None:
            print("Local model returned an error.")
            continue
